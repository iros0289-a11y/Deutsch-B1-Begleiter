...
function render() {
  ...
  if (!views.some(([id]) => id === state.view)) state.view = "dashboard";
  const renderer = {
  ...
  return `
    <section class="grid">
      <div class="card span-12">
        ...
        <div class="actions">
          <button class="secondary" id="playAllListening" type="button">Alle Hörtexte nacheinander abspielen</button>
...
function renderExamQuestion(item, index, section) {
  ...
  const audio = item.type === "listen" ? `<button class="ghost" type="button" data-play="${escapeHtml(item.audio)}">Audio</button>` : "";
  return `
...
function bindExams() {
  document.querySelector("#startExam")?.addEventListener("click", () => {
    state.exam = buildExam(Number(document.querySelector("#examNumber").value));
    startExamTimer();
    render();
  });
  ...
  document.querySelectorAll("[data-play]").forEach((button) => {
    button.addEventListener("click", () => speak(button.dataset.play));
  });
